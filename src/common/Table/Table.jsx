	...
	configs.map((item, index) => {
		...
		return (
			<Th
				...
				onClick={
					item.cellType !== TABLE_CELL_TYPE.CHECKBOX ? item.onHeaderClick : null
				}
				...
				content = (
					<Badge colorScheme={getStatusColorCode(row[col.cellProp])}>
						<Tooltip label={col?.message(row) ?? ""}>
							<Text>
								{col?.mapValue(row[col.cellProp], row, rowIndex) ??
                  row[col.cellProp]}
							</Text>
						</Tooltip>
					</Badge>
...
const DataTable = memo(
	({ colConfigs = [], rows = [], loading, noResultText = "", error }) => (
